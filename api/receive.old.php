...
if (!$stmt) {
    echo json_encode(["status"=>"error","message"=>"Prepare failed: ".$mysqli->error, "sql"=>$sql]);
    exit();
}
...
if (!$stmt->execute()) {
    ...
    echo json_encode(["status"=>"success","message"=>"Inserted/Updated successfully","rows_affected"=>$stmt->affected_rows]);
}


file_put_contents(__DIR__."/log_db.txt", date("Y-m-d H:i:s")." - JSON: ".$json." - SQL: ".$sql."\n", FILE_APPEND);
*/
