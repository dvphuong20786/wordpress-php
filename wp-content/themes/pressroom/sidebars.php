...
function pr_theme_sidebars_init()
{
	$labels = array(
		...
		'add_new' => _x('Add New', 'pressroom_sidebar', 'pressroom'),
	...
	@register_post_type("pressroom_sidebars", $args);
}  
...
function pr_theme_inner_sidebars_custom_box_main($post)
{
	...
	wp_nonce_field(plugin_basename( __FILE__ ), "pressroom_sidebars_noncename");
...
function pr_theme_save_sidebars_postdata($post_id) 
{
	...
	if((isset($_POST['pressroom_sidebars_noncename']) && !wp_verify_nonce($_POST['pressroom_sidebars_noncename'], plugin_basename( __FILE__ ))) || !isset($_POST['pressroom_sidebars_noncename']))
		return;
...
add_filter("manage_edit-pressroom_sidebars_columns", "pressroom_sidebars_edit_columns");

function pr_manage_pressroom_sidebars_posts_custom_column($column)
{
	global $post;
	switch($column)
	{
...
add_action("manage_pressroom_sidebars_posts_custom_column", "pr_manage_pressroom_sidebars_posts_custom_column");
...
add_filter("manage_edit-pressroom_sidebars_sortable_columns", "pressroom_sidebars_sortable_columns");
...
add_filter("request", "pressroom_sidebars_column_orderby");
?>