...
class pr_cart_icon_widget extends WP_Widget 
{
	...
	function __construct()
	{
		$widget_options = array(
			...
			'description' => __('Displays Shop Cart Icon', 'pressroom')
		);
    ...
    function widget($args, $instance) 
	{
		...
		if(is_plugin_active('woocommerce/woocommerce.php'))
		{
			...
			?>
			<a <?php echo ($icon_target=="new_window" ? " target='_blank'" : ""); ?>href="<?php echo esc_url($cart_url);?>" class="cart_icon <?php echo (isset($_COOKIE['pr_social_icons']) && $_COOKIE['pr_social_icons']!="" ? esc_attr($_COOKIE['pr_social_icons']) : esc_attr($icon_style));?>">&nbsp;<?php if($cart_items_number=="yes"): ?><span class="cart_items_number"<?php echo (!(int)$woocommerce->cart->cart_contents_count ? ' style="display: none;"' : ''); ?>><?php echo esc_html($woocommerce->cart->cart_contents_count); ?></span><?php endif;?></a>
			<?php
		}
        echo $after_widget;
    }
...
//register widget
function pr_cart_icon_widget_init()
{
	return register_widget("pr_cart_icon_widget");
}
add_action('widgets_init', 'pr_cart_icon_widget_init');
?>