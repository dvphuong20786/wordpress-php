...
class pr_scrolling_recent_posts_widget extends WP_Widget 
{
	...
	function __construct()
	{
		$widget_options = array(
			...
			'description' => __('Displays scrolling recent posts list', 'pressroom')
		);
		...
		foreach($posts_list as $post) 
		{
			$output .= '<li class="post">
				...
				if((int)$show_post_icon)
				{
					...
					$output .= (($is_review=="percentage" || $is_review=="points") && $post_format!="video" && $post_format!="gallery" ? '<span class="icon' . ($type=='vertical' ? ' review small">' : '"><span>' . get_post_meta($post->ID, "pressroom_review_average", true) . ($is_review=="percentage" ? '%' : '') . '</span>') . '</span>' : '') . ($post_format=="video" || $post_format=="gallery" ? '<span class="icon ' . esc_attr($post_format) . ($type=='vertical' ? ' small' : '') . '"></span>' : '');
				}
			$output .= get_the_post_thumbnail($post->ID, ($featured_image_size!="default" ? $featured_image_size : "pressroom-small-thumb") , array("alt" => get_the_title(), "title" => "")) .
				'</a>';
				if($type=="vertical")
	...
	function form($instance) 
	{
		...
		<p>
			...
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id('ids')); ?>" name="<?php echo esc_attr($this->get_field_name('ids')); ?>" type="text" value="<?php echo (is_array($ids) ? esc_attr(implode(",", $ids)) : ''); ?>" />
		...
		<p>
			...
			<select id="<?php echo esc_attr($this->get_field_id('order_by')); ?>" name="<?php echo esc_attr($this->get_field_name('order_by')); ?>">
				...
				<option value="rand"<?php echo ($order_by=="rand" ? ' selected="selected"' : ''); ?>><?php _e('Random', 'pressroom'); ?></option>
			</select>
		</p>
		<p>
...
//register widget
function pr_scrolling_recent_posts_widget_init()
{
	return register_widget("pr_scrolling_recent_posts_widget");
}
add_action('widgets_init', 'pr_scrolling_recent_posts_widget_init');
?>