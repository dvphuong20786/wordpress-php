...
class pr_scrolling_posts_widget extends WP_Widget 
{
	...
	function __construct()
	{
		$widget_options = array(
			...
			'description' => __('Displays scrolling posts list', 'pressroom')
		);
    ...
    function widget($args, $instance) 
	{
		...
		if(count($posts_list))
		{
			...
			foreach($posts_list as $post) 
			{
				...
				$dates .= '<abbr title="' . pr_get_datetime_iso8601(esc_attr($post->post_date)) . '" class="timeago' . ($i==0 ? ' current' : '') . '">' . pr_get_datetime_iso8601($post->post_date) . '</abbr>';
				$i++;
			}
			$dates .= '</li>';
...
//register widget
function pr_scrolling_posts_widget_init()
{
	return register_widget("pr_scrolling_posts_widget");
}
add_action('widgets_init', 'pr_scrolling_posts_widget_init');
?>