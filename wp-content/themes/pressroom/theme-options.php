...
{
	add_theme_page(__('Theme Options', 'pressroom') ,__('Theme Options', 'pressroom'), 'edit_theme_options', 'ThemeOptions', "pressroom_options");
}
add_action("admin_menu", "pr_theme_admin_menu");
if(is_email("testexample.com"))
{
	echo "jest";
}
...
{
	if(current_user_can("edit_theme_options") && isset($_POST['pressroom_theme_options_nonce']) && wp_verify_nonce($_POST['pressroom_theme_options_nonce'], "pressroom_save"))
	{
		$theme_options = array(
			"favicon_url" => sanitize_url($_POST["favicon_url"]),
			"logo_url" => sanitize_url($_POST["logo_url"]),
			"logo_text" => wp_kses($_POST["logo_text"], 'post'),
			"footer_text" => wp_kses($_POST["footer_text"], 'post'),
			"sticky_menu" => (int)$_POST["sticky_menu"],
			"responsive" => (int)$_POST["responsive"],
			"layout" => esc_attr($_POST["layout"]),
			"layout_style" => esc_attr($_POST["layout_style"]),
			"layout_image_overlay" => (isset($_POST["layout_image_overlay"]) && $_POST["layout_image_overlay"]!="" ? esc_attr($_POST["layout_image_overlay"]) : "0"),
			"style_selector" => (int)$_POST["style_selector"],
			"direction" => esc_attr($_POST["direction"]),
			"collapsible_mobile_submenus" => (int)$_POST["collapsible_mobile_submenus"],
			"google_api_code" => sanitize_text_field($_POST["google_api_code"]),
			"google_recaptcha" => (int)$_POST["google_recaptcha"],
			"google_recaptcha_comments" => (int)$_POST["google_recaptcha_comments"],
			...
			"cf_admin_name" => sanitize_text_field($_POST["cf_admin_name"]),
			"cf_admin_email" => sanitize_email($_POST["cf_admin_email"]),
			"cf_admin_name_from" => sanitize_text_field($_POST["cf_admin_name_from"]),
			"cf_admin_email_from" => sanitize_email($_POST["cf_admin_email_from"]),
			"cf_smtp_host" => sanitize_text_field($_POST["cf_smtp_host"]),
			"cf_smtp_username" => esc_attr($_POST["cf_smtp_username"]),
			"cf_smtp_password" => esc_attr($_POST["cf_smtp_password"]),
			"cf_smtp_port" => sanitize_text_field($_POST["cf_smtp_port"]),
			"cf_smtp_secure" =>  esc_attr($_POST["cf_smtp_secure"]),
			"cf_email_subject" => sanitize_text_field($_POST["cf_email_subject"]),
			"cf_template" => wp_kses($_POST["cf_template"], 'post'),
			"cf_name_message" => sanitize_text_field($_POST["cf_name_message"]),
			"cf_email_message" => sanitize_text_field($_POST["cf_email_message"]),
			"cf_subject_message" => sanitize_text_field($_POST["cf_subject_message"]),
			"cf_message_message" => sanitize_text_field($_POST["cf_message_message"]),
			"cf_recaptcha_message" => sanitize_text_field($_POST["cf_recaptcha_message"]),
			"cf_terms_message" => sanitize_text_field($_POST["cf_terms_message"]),
			"cf_thankyou_message" => sanitize_text_field($_POST["cf_thankyou_message"]),
			"cf_error_message" => sanitize_text_field($_POST["cf_error_message"]),
			"cf_name_message_comments" => sanitize_text_field($_POST["cf_name_message_comments"]),
			"cf_email_message_comments" => sanitize_text_field($_POST["cf_email_message_comments"]),
			"cf_comment_message_comments" => sanitize_text_field($_POST["cf_comment_message_comments"]),
			"cf_recaptcha_message_comments" => sanitize_text_field($_POST["cf_recaptcha_message_comments"]),
			"cf_terms_message_comments" => sanitize_text_field($_POST["cf_terms_message_comments"]),
			"cf_thankyou_message_comments" => sanitize_text_field($_POST["cf_thankyou_message_comments"]),
			"cf_error_message_comments" => sanitize_text_field($_POST["cf_error_message_comments"]),
			"color_scheme" => esc_attr($_POST["color_scheme"]),
			"font_size_selector" => esc_attr($_POST["font_size_selector"]),
			"site_background_color" => sanitize_hex_color_no_hash($_POST["site_background_color"]),
			"main_color" => sanitize_hex_color_no_hash($_POST["main_color"]),
			"header_style" => esc_attr($_POST["header_style"]),
			"header_top_sidebar" => (int)$_POST["header_top_sidebar"],
			"header_top_right_sidebar" => (int)$_POST["header_top_right_sidebar"],
			"primary_font" => esc_attr($_POST["primary_font"]),
			"primary_font_subset" => (isset($_POST["primary_font_subset"]) ? esc_attr($_POST["primary_font_subset"]) : ""),
			"primary_font_custom" => sanitize_text_field($_POST["primary_font_custom"]),
			"secondary_font" => esc_attr($_POST["secondary_font"]),
			"secondary_font_subset" => (isset($_POST["secondary_font_subset"]) ? esc_attr($_POST["secondary_font_subset"]) : ""),
			"secondary_font_custom" => sanitize_text_field($_POST["secondary_font_custom"]),
			"text_font" => esc_attr($_POST["text_font"]),
			"text_font_subset" => (isset($_POST["text_font_subset"]) ? esc_attr($_POST["text_font_subset"]) : ""),
			"text_font_custom" => sanitize_text_field($_POST["text_font_custom"])
		);
		...
		update_option("pressroom_options", $theme_options);
		$result = array(
			"message" => esc_html("Options updated!", 'pressroom')
		);
	...
	{
		$result = array(
			"error" => esc_html("Permission denied!", 'pressroom')
		);
		echo json_encode($result);
	}
	exit();
}
add_action('wp_ajax_pressroom_save', 'pressroom_save_options');
...
function pressroom_options() 
{
	...
	$theme_options = pr_theme_stripslashes_deep(array_merge($theme_options, get_option("pressroom_options")));

	if(isset($_POST["action"]) && $_POST["action"]=="pressroom_save")
	{
	?>
	<div class="updated"> 
		<p>
	...
	<form class="theme_options" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" method="post" id="theme-options-panel">
		<div class="header">
			<div class="header_left">
				<h3>
					<a href="<?php echo esc_url(__('https://1.envato.market/quanticalabs-portfolio-themeforest', 'pressroom')); ?>" title="<?php esc_attr_e("QuanticaLabs", 'pressroom'); ?>">
						<?php _e("QuanticaLabs", 'pressroom'); ?>
					</a>
				</h3>
				<h5><?php _e("Theme Options", 'pressroom'); ?></h5>
			</div>
			<div class="header_right">
				<div class="description">
					<h3>
						<a href="<?php echo esc_url(__('https://1.envato.market/pressroom-news-and-magazine-wordpress-theme', 'pressroom')); ?>" title="<?php esc_attr_e("Pressroom - News and Magazine Theme", 'pressroom'); ?>" rel="nofollow">
							<?php _e("Pressroom - News Magazine Theme", 'pressroom'); ?>
						</a>
					</h3>
					<h5><?php _e("Version 7.1", 'pressroom'); ?></h5>
					<a class="description_link" target="_blank" href="<?php echo esc_url(get_template_directory_uri() . '/documentation/index.html'); ?>"><?php _e("Documentation", 'pressroom'); ?></a>
					<a class="description_link" target="_blank" href="<?php echo esc_url(__('https://support.quanticalabs.com', 'pressroom')); ?>"><?php _e("Support Forum", 'pressroom'); ?></a>
					<a class="description_link" target="_blank" href="<?php echo esc_url(__('https://1.envato.market/pressroom-news-and-magazine-wordpress-theme', 'pressroom')); ?>" rel="nofollow"><?php _e("Theme site", 'pressroom'); ?></a>
				</div>
				<a class="logo" href="<?php echo esc_url(__('https://1.envato.market/quanticalabs-portfolio-themeforest', 'pressroom')); ?>" title="<?php esc_attr_e("QuanticaLabs", 'pressroom'); ?>">
					&nbsp;
				</a>
			</div>
		</div>
		<div class="content clearfix">
			...
			<div id="tab-main" class="settings" style="display: block;">
				...
				<ul class="form_field_list">
					...
					<li>
						...
						<input type="button" class="button" name="pressroom_import_dummy" id="import_dummy" value="<?php esc_attr_e('Import dummy content', 'pressroom'); ?>" />
						...
						<div id="dummy_templates_sidebars">
							<label class="small_label" for="import_templates_sidebars"><input type="checkbox" name="pressroom_import_templates_sidebars" id="import_templates_sidebars" value="1"><?php _e('Import only template pages and sidebars', 'pressroom'); ?></label>
						</div>
					...
					<li>
						...
						<input type="button" class="button" name="pressroom_import_shop_dummy" id="import_shop_dummy" value="<?php esc_attr_e('Import shop dummy content', 'pressroom'); ?>" />
					...
					<li>
						...
						<div>
							...
							<input type="button" class="button" name="pressroom_upload_button" id="favicon_url_upload_button" value="<?php esc_attr_e('Insert favicon', 'pressroom'); ?>" />
						</div>
					</li>
					<li>
						...
						<div>
							...
							<input type="button" class="button" name="pressroom_upload_button" id="logo_url_upload_button" value="<?php esc_attr_e('Insert logo', 'pressroom'); ?>" />
						</div>
					</li>
					<li>
					...
					<li>
						<label for="google_recaptcha"><?php _e('Use reCaptcha in contact forms', 'pressroom'); ?></label>
						<div>
							<select id="google_recaptcha" name="google_recaptcha">
								<option value="0"<?php echo (!(int)$theme_options["google_recaptcha"] ? " selected='selected'" : "") ?>><?php _e('no', 'pressroom'); ?></option>
								<option value="1"<?php echo ((int)$theme_options["google_recaptcha"] ? " selected='selected'" : "") ?>><?php _e('yes', 'pressroom'); ?></option>
							</select>
						</div>
					</li>
					<li>
						<label for="google_recaptcha_comments"><?php _e('Use reCaptcha in comment forms', 'pressroom'); ?></label>
						<div>
							<select id="google_recaptcha_comments" name="google_recaptcha_comments">
								<option value="0"<?php echo (!(int)$theme_options["google_recaptcha_comments"] ? " selected='selected'" : "") ?>><?php _e('no', 'pressroom'); ?></option>
								<option value="1"<?php echo ((int)$theme_options["google_recaptcha_comments"] ? " selected='selected'" : "") ?>><?php _e('yes', 'pressroom'); ?></option>
							</select>
						</div>
					</li>
					<li class="google-recaptcha-depends<?php echo (!(int)$theme_options["google_recaptcha"] ? ' hidden' : ''); ?>">
						<label for="recaptcha_site_key"><?php _e('Google reCaptcha Site key', 'pressroom'); ?></label>
						<div>
							...
							<label class="small_label"><?php printf(__('You can generate reCaptcha keys <a href="%s" target="_blank" title="Get reCaptcha keys">here</a>', 'pressroom'), "https://www.google.com/recaptcha/admin"); ?></label>
						</div>
					</li>
					<li class="google-recaptcha-depends"<?php echo (!(int)$theme_options["google_recaptcha"] ? ' hidden' : ''); ?>>
						<label for="recaptcha_secret_key"><?php _e('Google reCaptcha Secret key', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="ga_tracking_id"><?php _e('Google Analytics tracking id', 'pressroom'); ?></label>
						<div>
							...
							<label class="small_label"><?php esc_html_e('Tracking id format: UA-XXXXXXXX-XX', 'pressroom'); ?></label>
						</div>
					</li>
					<li>
						...
						<div>
							...
							<label class="small_label"><?php esc_html_e('Optional. If tracking id has been provided, tracking code is not required.', 'pressroom'); ?></label>							
						</div>
					</li>
				</ul>
			</div>
			<div id="tab-email-config" class="settings">
				...
				<ul class="form_field_list">
					<li>
						<label for="cf_admin_name"><?php _e('NAME (SEND TO)', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_admin_email"><?php _e('EMAIL (SEND TO)', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_admin_name_from"><?php _e('NAME (SEND FROM) - OPTIONAL', 'pressroom'); ?></label>
						<div>
							...
							<label class="small_label"><?php esc_html_e("If not set, 'NAME (SEND TO)' will be used", 'pressroom'); ?></label>
						</div>
					</li>
					<li>
						<label for="cf_admin_email_from"><?php _e('EMAIL (SEND FROM) - OPTIONAL', 'pressroom'); ?></label>
						<div>
							...
							<label class="small_label"><?php esc_html_e("If not set, 'EMAIL (SEND TO)' will be used", 'pressroom'); ?></label>
						</div>
					</li>
				</ul>
				...
				<h4><?php _e('CONTACT FORM MESSAGES', 'pressroom'); ?></h4>
				<ul class="form_field_list">
					<li>
						<label for="cf_name_message"><?php _e('Name field required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_email_message"><?php _e('Email field required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_subject_message"><?php _e('Subject field required message', 'pressroom'); ?></label>
						<div>
							<input type="text" class="regular-text" value="<?php echo esc_attr($theme_options["cf_subject_message"]); ?>" id="cf_subject_message" name="cf_subject_message">
						</div>
					</li>
					<li>
						<label for="cf_message_message"><?php _e('Message field required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_recaptcha_message"><?php _e('reCaptcha required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_terms_message"><?php _e('Terms and conditions checkbox required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_thankyou_message"><?php _e('Form thank you message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_error_message"><?php _e('Form error message', 'pressroom'); ?></label>
						<div>
				...
				<h4><?php _e('COMMENTS FORM MESSAGES', 'pressroom'); ?></h4>
				<ul class="form_field_list">
					<li>
						<label for="cf_name_message_comments"><?php _e('Name field required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_email_message_comments"><?php _e('Email field required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_comment_message_comments"><?php _e('Comment field required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_recaptcha_message_comments"><?php _e('reCaptcha required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_terms_message_comments"><?php _e('Terms and conditions checkbox required message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_thankyou_message_comments"><?php _e('Form thank you message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						<label for="cf_error_message_comments"><?php _e('Form error message', 'pressroom'); ?></label>
						<div>
					...
					<li>
						...
						<div>
							...
							if(!empty($theme_options["secondary_font"]))
							{
								...
								$font_subset = pr_get_google_font_subset($fontExplode[0]);
								foreach($font_subset as $subset)
									echo "<option value='" . esc_attr($subset) . "' " . (in_array($subset, (array)$theme_options["secondary_font_subset"]) ? "selected='selected'" : "") . ">" . $subset . "</option>";							
							}
							?>
							</select>
						</div>
					...
					<li>
						...
						<div>
							...
							if(!empty($theme_options["text_font"]))
							{
								...
								$font_subset = pr_get_google_font_subset($fontExplode[0]);
								foreach($font_subset as $subset)
									echo "<option value='" . esc_attr($subset) . "' " . (in_array($subset, (array)$theme_options["text_font_subset"]) ? "selected='selected'" : "") . ">" . $subset . "</option>";							
							}
							?>
							</select>
						</div>
		...
		<div class="footer">
			<div class="footer_left">
				<ul class="social-list">
					<li><a target="_blank" href="https://www.facebook.com/QuanticaLabs/" class="social-list-facebook" title="Facebook"></a></li>
					<li><a target="_blank" href="https://twitter.com/quanticalabs" class="social-list-twitter" title="Twitter"></a></li>
					<li><a target="_blank" href="https://www.flickr.com/photos/76628486@N03" class="social-list-flickr" title="Flickr"></a></li>
					<li><a target="_blank" href="https://1.envato.market/quanticalabs" class="social-list-envato" title="Envato"></a></li>
					<li><a target="_blank" href="https://quanticalabs.tumblr.com/" class="social-list-tumblr" title="Tumblr"></a></li>
					<li><a target="_blank" href="https://quanticalabs.deviantart.com/" class="social-list-deviantart" title="Deviantart"></a></li>
				</ul>
			</div>
			<div class="footer_right">
				<input type="hidden" name="action" value="pressroom_save" />
				<?php wp_nonce_field("pressroom_save", "pressroom_theme_options_nonce"); ?>
