...
class Pr_Post
{
	...
	public function __construct($blog_type, $featured_post, $is_review, $post_format, $featured_image_size, $show_post_icon, $show_post_date, $show_post_categories, $show_post_excerpt, $show_post_author, $post_details_layout, $post_list_style, $category_filter, $i)
	{
	...
	public function getThumbnail($default_image_size = null)
	{
		...
		if(has_post_thumbnail())
		{
			if($this->blog_type=="small")
				$output .= '<a class="post_image" href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . ($this->show_post_icon ? (($this->is_review=="percentage" || $this->is_review=="points") && $this->post_format!="video" && $this->post_format!="gallery" ? '<span class="icon small review"></span>' : '') . ($this->post_format=="video" || $this->post_format=="gallery" ? '<span class="icon' . ($this->featured_post!="-" && $this->i>0 ? ' small' : '') . ' ' . esc_attr($this->post_format) . '"></span>' : '') : '') . get_the_post_thumbnail(get_the_ID(), ($this->featured_image_size!="default" ? $this->featured_image_size : "pressroom-small-thumb"), array("alt" => get_the_title(), "title" => "")) . '</a>';	
			else
				$output .= '<a class="post_image" href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">' . ($this->show_post_icon ? (($this->is_review=="percentage" || $this->is_review=="points") && $this->post_format!="video" && $this->post_format!="gallery" ? ($this->featured_post!="-" && $this->i>0 ? '<span class="icon small review"></span>' : '<span class="icon"><span>' . get_post_meta(get_the_ID(), "pressroom_review_average", true) . ($this->is_review=="percentage" ? '%' : '') . '</span></span>') : '') . ($this->post_format=="video" || $this->post_format=="gallery" ? '<span class="icon' . ($this->featured_post!="-" && $this->i>0 ? ' small' : '') . ' ' . esc_attr($this->post_format) . '"></span>' : '') : '') . get_the_post_thumbnail(get_the_ID(), ($this->featured_image_size!="default" ? $this->featured_image_size : ($this->featured_post!="-" && $this->i>0 ? "pressroom-small-thumb" : $default_image_size)), array("alt" => get_the_title(), "title" => "")) . '</a>';
		}
		return $output;
	}
	...
	public function getPostDetails()
	{
		...
		if($this->show_post_date || $this->show_post_categories)
		{
			...
			if($this->show_post_categories)
			{
				...
				$primary_category = get_post_meta(get_the_ID(), "pressroom_primary_category", true);
	...
	public static function getReviewAverage($post_id=null, $is_review=null)
	{
		if($post_id==null)
			$post_id = get_the_ID();
		if($is_review==null)
		...
		{
			$review_label = get_post_meta($post_id, "pressroom_review_label", true);
			$review_value = get_post_meta($post_id, "pressroom_review_value", true);
