...
function pr_theme_rank_list_shortcode($atts)
{
	...
	{
		if(($type=="views" && pr_getPostViews($post->ID)>0) || ($type=="comment_count" && get_comments_number($post->ID)>0))
		{
			...
			if($i==0)
			{
				...
				if((int)$show_post_icon)
				{
					...
					$output .= (($is_review=="percentage" || $is_review=="points") && $post_format!="video" && $post_format!="gallery" ? '<span class="icon"><span>' . get_post_meta($post->ID, "pressroom_review_average", true) . ($is_review=="percentage" ? '%' : '') . '</span></span>' : '') . ($post_format=="video" || $post_format=="gallery" ? '<span class="icon ' . esc_attr($post_format) . ($type=='vertical' ? ' small' : '') . '"></span>' : '');
				}
			...
			$output .= '<div class="post_content">
				<span class="number animated_element" data-value="' . ($type=="views" ? pr_getPostViews($post->ID) : get_comments_number($post->ID)) . '"></span>
