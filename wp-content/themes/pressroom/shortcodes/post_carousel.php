...
function pr_theme_post_carousel_shortcode($atts)
{
	...
	if($order_by=="rate")
		$args['meta_key'] = "pressroom_review_average";
	...
	foreach($posts_list as $post) 
	{
		...
		$output .= get_the_post_thumbnail($post->ID, ($featured_image_size!="default" ? $featured_image_size : "pressroom-small-thumb") , array("alt" => get_the_title(), "title" => "")) .
			...
			if((int)$read_more)
				$output .= '<a title="' . esc_attr__('Read more', 'pressroom') . '" href="' . esc_url(get_permalink($post->ID)) . '" class="read_more"><span class="arrow"></span><span>' . __('Read more', 'pressroom') . '</span></a>';
			if($type=="vertical")
				$output .= '</div>';
		$output .= '</li>';
		$i++;
...
function pr_theme_post_carousel_vc_init()
{
	...
	$params = array(
		...
		array(
			...
			"param_name" => "order_by",
			"value" => array(__("Date", 'pressroom') => "date", __("Title, menu order", 'pressroom') => "title menu_order", __("Menu order", 'pressroom') => "menu_order", __("Post views", 'pressroom') => "views", __("Comment count", 'pressroom') => "comment_count", __("Rate", 'pressroom') => "rate", __("Random", 'pressroom') => "rand")
		),
		array(
			"type" => "dropdown",
			"class" => "",
