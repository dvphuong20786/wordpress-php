...
{
	if(wp_doing_ajax() && (!isset($_POST['pr_ajax_nonce']) || !wp_verify_nonce($_POST['pr_ajax_nonce'], 'pr-ajax-nonce')))
	{
		echo "blog_posts_start" . esc_html("Nonce verification failed!", 'pressroom') . "blog_posts_end";
		return false;
	}
	if(isset($_POST["atts"]))
	{
	...
	extract(shortcode_atts(array(
		...
		"order_by" => "date",
		"order" => "DESC",
		"featured_post_display" => "selected",
		"featured_post" => "-",
	...
	if($featured_post_display=="selected" && $featured_post!="-")
	...
	$args = array( 
		'post__in' => $ids,
		'post__not_in' => ($featured_post_display=="selected" && $featured_post!="-" ? array((int)$featured_post) : array()),
		'post_type' => 'post',
		'post_status' => 'publish',
		'paged' => $paged,
	...
	if($author_current)
	{
		...
		$atts['ignore_sticky_posts'] = $args['ignore_sticky_posts'];
	}
	if($order_by=="views")
	...
	if($featured_post_display=="first")
	{
		$args['paged'] = 1;
		$args['posts_per_page'] = 1;
		$featured_post_array = get_posts($args);
		if(count($featured_post_array))
		{
			$featured_post_object = $featured_post_array[0];
			$featured_post = $featured_post_object->ID;
			$args['post__not_in'] = ((int)$featured_post ? array((int)$featured_post) : array());
		}
		$args['paged'] = $paged;
	...
	{
		array_unshift($blog_query->posts, $featured_post_object);
		$blog_query->post_count++;
	}
	...
	if($blog_query->have_posts())
	{
		...
		while ($blog_query->have_posts()) : $blog_query->the_post();
			$post = new Pr_Post("medium", $featured_post, get_post_meta(get_the_ID(), "pressroom_is_review", true), get_post_format(get_the_ID()), $featured_image_size, (int)$show_post_icon, (int)$show_post_date, (int)$show_post_categories, (int)$show_post_excerpt, (int)$show_post_author, $post_details_layout, $post_list_style, $category, $i);
			...
			else
			{
				...
				if(($featured_post!="-" && $i==0 && (int)$read_more_featured) || ((int)$read_more && ($i>0 || $featured_post=="-")))
					$output .= '<a title="' . esc_attr__('READ MORE', 'pressroom') . '" href="' . esc_url(get_permalink()) . '" class="read_more"><span class="arrow"></span><span>' . __('READ MORE', 'pressroom') . '</span></a>';
				if($featured_post!="-" && $i>0)
					$output .= '</div>';
			}
			$output .= '</li>';
		...
		if(($show_more_page!="-" || $show_more_custom_url!="") && $featured_post!="-")
			$output .= '<a title="' . esc_attr($show_more_label) . '" href="' . ($show_more_page!="-" ? esc_url(get_permalink($show_more_page)) : esc_url($show_more_custom_url)) . '" class="more page_margin_top">' . $show_more_label . '</a>';
		$output .= '</div>';
	}
	else if(is_search())
	{
	...
	if(isset($_POST["action"]) && $_POST["action"]=="theme_" . $component . "_pagination")
	{
		...
		//return HTML code
		echo "page_number_start" . $page_number . "page_number_endblog_posts_start" . $output . "blog_posts_endblog_pagination_start" . pr_pagination(true, '', 2, false, 'theme_' . $component . '_pagination', 'page_margin_top', $blog_query) . "blog_pagination_end";
	...
	{
		pr_get_theme_file("/pagination.php");
		$output .= pr_pagination(((int)$ajax_pagination ? true : false), '', 2, false, 'theme_' . $component . '_pagination', 'page_margin_top', $blog_query);
		$atts["page_uri"] = (!is_front_page() ?  $_SERVER["REQUEST_URI"] : "/");
		if((int)$ajax_pagination)
			$output .= '<input type="hidden" name="theme_' . esc_attr($component) . '_pagination" value="' . htmlentities(json_encode($atts)) . '" />';
	}
	//Reset Postdata
	wp_reset_postdata();
	return $output;
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				...
				"param_name" => "order_by",
				"value" => array(__("Date", 'pressroom') => "date", __("Title, menu order", 'pressroom') => "title menu_order", __("Menu order", 'pressroom') => "menu_order", __("Post views", 'pressroom') => "views", __("Comment count", 'pressroom') => "comment_count", __("Random", 'pressroom') => "rand")
			),
			array(
				"type" => "dropdown",
				"class" => "",
			...
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Display featured post", 'pressroom'),
				"param_name" => "featured_post_display",
				"value" => array(__("Yes, display custom selected post", 'pressroom') => "selected", __("Yes, display first post as featured", 'pressroom') => "first", __("No", 'pressroom') => 0)
			),
			array(
				...
				"dependency" => Array('element' => "featured_post_display", 'value' => array("selected"))
			),
			array(
				"type" => "dropdown",
				"class" => "",
				...
				"dependency" => Array('element' => "featured_post_display", 'value' => array("selected", "first"))
