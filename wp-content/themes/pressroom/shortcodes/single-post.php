...
function pr_theme_single_post($atts, $content)
{
	...
	if((int)$show_post_categories || (int)$show_post_date || (int)$show_post_author || (int)$show_post_views || (int)$show_post_comments)
	{
		...
		if((int)$show_post_date && isset($post))
		...
		if((int)$show_post_views)
			$output .= '<li class="detail views">' . pr_getPostViews($post->ID) . __(" Views", 'pressroom') . '</li>';
		if((int)$show_post_comments)
		{	
			$comments_count = (isset($post) ? wp_count_comments($post->ID) : null);
			$comments_count = ($comments_count!=null ? $comments_count->approved : 0);
	...
	$is_review = (isset($post) ? get_post_meta($post->ID, "pressroom_is_review", true) : "");
	...
	if($post_format=="video" && get_post_meta($post->ID, "pressroom_video_url", true)!="")
	{
		$output .= '<iframe class="iframe_video page_margin_top" src="' . esc_url(get_post_meta($post->ID, "pressroom_video_url", true)) . '" allowfullscreen></iframe>';
	}
	else if($post_format=="audio" && get_post_meta($post->ID, "pressroom_audio_url", true)!="")
	{
		$output .= '<iframe class="iframe_sound page_margin_top" src="' . esc_url(get_post_meta($post->ID, "pressroom_audio_url", true)) . '"></iframe>';
	}
	else if($post_format=="gallery")
	{
		$attachment_ids = get_post_meta($post->ID, "pressroom_attachment_ids", true);
		$images = get_post_meta($post->ID, "pressroom_images", true);
		...
		/*if(is_rtl())
		{
			...
			{
				$gallery_control .= '<li><a href="#">' . wp_get_attachment_image($attachment_ids[$i], "pressroom-gallery-thumb-type-3") . '</a></li>';
				$gallery_carousel .= '<li>
			...
			for($i=0; $i<count($images); $i++)
			{
				$output .= '<li>
								...
								<div class="vc_col-sm-4 wpb_column vc_column_container">
									...
									<div class="clearfix">
										<a class="read_more close_popup page_margin_top_section" href="#" title="' . esc_attr__("Return to post", 'pressroom') . '"><span class="arrow"></span><span>' . __("RETURN TO POST", 'pressroom') . '</span></a>
									</div>
									' . $icons_html . '
								</div>
							</li>';
	...
	if($is_review!="" && $is_review!="no")
	{
		...
		{
			$output .= '<div class="review_summary margin_top_10 clearfix" itemprop="reviewRating" itemscope itemtype="https://schema.org/Rating">
	...
	$featured_caption = get_post_meta(get_the_ID(), "pressroom_featured_caption", true);
	if($featured_caption!="")
	{
		$output .= '<div class="sentence">
			<span class="text">' . $featured_caption . '</span>';
		$featured_caption_author = get_post_meta(get_the_ID(), "pressroom_featured_caption_author", true);
	...
	if($show_share_box && (int)$icons_count)
	{
		...
		for($i=0; $i<$icons_count; $i++)
		{
			if(isset($atts["icon_type" . $i]))
			{
				...
				$output .= '<li><a' . (isset($atts["icon_target" . $i]) && $atts["icon_target" . $i]=="_blank" ? ' target="_blank"' : '') . ' title="" href="' . esc_url(str_replace("{MEDIA_URL}", $large_image_url, str_replace("{TITLE}", urlencode(get_the_title()), str_replace("{URL}", esc_url(get_permalink()), $atts["icon_url" . $i])))) . '" class="social_icon ' . esc_attr($atts["icon_type" . $i]) . '">&nbsp;</a></li>';
			}
		}
		$output .= '</ul></div></div>';
	}
...
function pr_theme_single_post_vc_init()
{
	...
	$image_sizes_array[__("full (original image resolution)", 'pressroom')] = "full";
	...
	for($i=0; $i<25; $i++)
	{
		...
		$params[] = array(
			...
			"value" => "",
			"dependency" => Array('element' => "show_share_box", 'value' => '1'),
			"description" => sprintf(__("Use <strong>{URL}</strong> constant to have current post url in the link. You can also use <strong>{TITLE}</strong> variable and {MEDIA_URL} variable. Example: https://www.facebook.com/sharer.php?u=<strong>{URL}</strong> You can use <a href='%s' target='_blank'>Share Link Generator</a> to create share link", 'pressroom'), esc_url(__("https://www.sharelinkgenerator.com/", 'pressroom')))
		);
		$params[] = array(
			"type" => "dropdown",
			"class" => "",
