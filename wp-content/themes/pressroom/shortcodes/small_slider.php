...
function pr_theme_small_slider_shortcode($atts)
{
	...
	foreach($posts_list as $post) 
	{
		...
		if((int)$show_post_icon)
		{
			...
			$output .= (($is_review=="percentage" || $is_review=="points") && $post_format!="video" && $post_format!="gallery" ? '<span class="icon"><span>' . get_post_meta($post->ID, "pressroom_review_average", true) . ($is_review=="percentage" ? '%' : '') . '</span></span>' : '') . ($post_format=="video" || $post_format=="gallery" ? '<span class="icon ' . esc_attr($post_format) . '"></span>' : '');
		}
