<?php
function pr_theme_get_posts_global_lists()
{
...
add_action("after_setup_theme", "pr_theme_get_posts_global_lists");
...
function pr_theme_shortcodes_vc_init()
{
	//row inner
	$attributes = array(
		array(
	...
	vc_map( array(
		...
		'params' => array(
			...
			array(
				...
				'param_name' => 'el_id',
				'description' => sprintf( __( 'Enter row ID (Note: make sure it is unique and valid according to <a href="%s" target="_blank">w3c specification</a>).', 'pressroom' ), 'https://www.w3schools.com/tags/att_global_id.asp' ),
			),
			array(
				'type' => 'checkbox',
	...
	vc_map( array(
		...
		'params' => array(
			...
			array(
				...
				'param_name' => 'el_id',
				'description' => sprintf( __( 'Enter element ID (Note: make sure it is unique and valid according to <a href="%s" target="_blank">w3c specification</a>).', 'pressroom' ), 'https://www.w3schools.com/tags/att_global_id.asp' ),
			),
			array(
				'type' => 'textfield',
				'heading' => __( 'Extra class name', 'pressroom' ),
...
{
	add_action("init", "pr_theme_shortcodes_vc_init");
}
...
/* Separator (Divider)
---------------------------------------------------------- */
function pr_theme_vc_separator_pr_vc_init()
{
	vc_map( array(
...
if(function_exists('vc_map'))
{
	add_action("init", "pr_theme_vc_separator_pr_vc_init");
}
...
function pr_theme_box_header_vc_init()
{
	vc_map( array(
...
if(function_exists('vc_map'))
{
	add_action("init", "pr_theme_box_header_vc_init");
}
...
//wpbakery page builder
function pr_theme_dropcap_vc_init()
{
...
if(function_exists('vc_map'))
{
	add_action("init", "pr_theme_dropcap_vc_init");
}
...
//wpbakery page builder
function pr_theme_read_more_button_vc_init()
{
	vc_map( array(
...
if(function_exists('vc_map'))
{
	add_action("init", "pr_theme_read_more_button_vc_init");
}
