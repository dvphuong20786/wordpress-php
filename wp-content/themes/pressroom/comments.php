...
if(have_comments()):
	...
	?>
	<h4 class="box_header"><?php echo esc_html($comments_count) . " " . ($comments_count!=1 ? __("Comments", 'pressroom') : __("Comment", 'pressroom')); ?></h4>
	<ul id="comments_list">
		<?php
...
?>
	<li <?php comment_class('comment clearfix'); ?> id="comment-<?php echo esc_attr(get_comment_ID()); ?>">
		...
		{
			echo '<a class="parent_arrow" href="#comment-' . (int)esc_attr($comment->comment_parent) . '" title="' . esc_attr__('Show comment', 'pressroom') . '"></a>';
		}
		?>
		...
		<div class="comment_details">
			<div class="posted_by clearfix">
				...
				{	
					...
					echo '<a href="#comment-' . (int)esc_attr($comment->comment_parent) . '" class="in_reply">@' . $parent_author . '</a>';
				}
				?>
				</h5>
			...
			?>
			<a class="read_more reply_button" href="#<?php echo esc_attr(get_comment_ID()); ?>" title="<?php esc_attr_e('Reply', 'pressroom'); ?>">
...
function pr_comments_pagination($range, $pages)
{
	...
	if($paged > 1 && $showitems < $pages) echo "<li class='left'><a href='#page-" . esc_attr(($paged-1)) . "'>&lsaquo;</a></li>";
	...
	if ($paged < $pages && $showitems < $pages) echo "<li class='right'><a href='#page-" . esc_attr(($paged+1)) . "'>&rsaquo;</a></li>";  
